...
pub enum ScheduleEvent {
    ...
    VoteFor(*const PeerId, *const Transaction),
}
...
impl ScheduleEvent {
    ...
    pub fn fail(&self) -> Option<*const PeerId> {
    ...
    pub fn get_peer(&self) -> *const PeerId {
        match *self {
...
pub struct PendingTransactions(BTreeMap<*const PeerId, Vec<*const Transaction>>);

impl PendingTransactions {
    pub fn new<R: Rng>(
        rng: &mut R,
        peers: &[*const PeerId],
        transactions: &Vec<*const Transaction>,
    ) -> PendingTransactions {
    ...
    pub fn next_for_peer(&mut self, peer: *const PeerId) -> Option<*const Transaction> {
        self.0.get_mut(peer).and_then(|trans| trans.pop())
    }

    /// Gets the next transaction for a given node without removing it
    pub fn peek_next_for_peer(&mut self, peer: *const PeerId) -> Option<*const Transaction> {
    ...
    pub fn nonempty_peers(&self) -> Vec<*const PeerId> {
        self.0
            .iter()
    ...
    pub fn remove_peers<I: IntoIterator<Item = *const PeerId>>(&mut self, peers: I) {
        for peer in peers {
    ...
    pub fn all_transactions(
        &mut self,
    ) -> impl Iterator<Item = (*const PeerId, *const Transaction)> {
...
impl Drop for PendingTransactions {
    fn drop(&mut self) {
        // TODO: drop all values
    }
}
